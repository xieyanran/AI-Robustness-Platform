...
def train(model, data, device, maxepoch, data_path = './', save_per_epoch = 10, seed = 100):
    ...
    Examples
    --------
    >>>import AIrobust.image.netmodels.train_model as trainmodel
    ...
    if (model == 'CNN'):
        import AIrobust.image.netmodels.CNN as MODEL
        #from AIrobust.image.netmodels.CNN import Net
        train_net = MODEL.Net().to(device)

    elif (model == 'ResNet18'):
        import AIrobust.image.netmodels.resnet as MODEL
        ...
        import AIrobust.image.netmodels.densenet as MODEL
        train_net = MODEL.densenet_cifar().to(device)

    elif (model == 'vgg11'):
        import AIrobust.image.netmodels.vgg as MODEL
