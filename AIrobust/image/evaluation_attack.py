...
from AIrobust.image import utils
...
def load_net(attack_model, filename, path):
    if(attack_model == "CNN"):
        from AIrobust.image.netmodels.CNN import Net

        model = Net()
    if(attack_model == "ResNet18"):
        import AIrobust.image.netmodels.resnet as Net
        model = Net.ResNet18()
...
def generate_dataloader(dataset, batch_size):
    if(dataset == "MNIST"):
        test_loader = torch.utils.data.DataLoader(
                        datasets.MNIST('AIrobust/image/data', train = False,
                        download = True,
        ...
        test_loader = torch.utils.data.DataLoader(
                        datasets.CIFAR10('AIrobust/image/data', train = False,
                        download = True,
                        transform = transforms.Compose([transforms.ToTensor()])),
                        batch_size = args.batch_size,
                        shuffle = True)
...
if __name__ == "__main__":
    ...
    if(args.attack_method == "PGD"):
        from AIrobust.image.attack.pgd import PGD
        ...
        from AIrobust.image.attack.fgsm import FGSM
        test_loader = generate_dataloader(args.dataset, args.batch_size)
        ...
        from AIrobust.image.attack.lbfgs import LBFGS
        try:
        ...
        from AIrobust.image.attack.cw import CarliniWagner
        ...
        from AIrobust.image.attack.deepfool import DeepFool
        ...
        from AIrobust.image.attack.onepixel import Onepixel
