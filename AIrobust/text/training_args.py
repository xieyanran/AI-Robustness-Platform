...
class TrainingArgs:
    ...
    Args:
        ...
        attack_num_workers_per_device (:obj:`int`, defaults to `optional`, :obj:`1`):
            Number of worker processes to run per device for attack. Same as :obj:`num_workers_per_device` argument for :class:`~text.AttackArgs`.
        ...
        wandb_project (:obj:`str`, `optional`, defaults to :obj:`"text"`):
...
class _CommandLineTrainingArgs:
    ...
    def _create_model_from_args(cls, args):
        ...
        if args.model_name_or_path == "lstm":
            logger.info("Loading text model: LSTMForClassification")
            ...
            logger.info("Loading text model: WordCNNForClassification")
            max_seq_len = args.model_max_length if args.model_max_length else 128
            num_labels = args.model_num_labels if args.model_num_labels else 2
    ...
    def _create_attack_from_args(cls, args, model_wrapper):
        ...
        assert isinstance(
            attack, Attack
        ), "`attack` must be of type `text.Attack`."
        return attack
