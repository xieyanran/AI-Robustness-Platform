...
class Trainer:
    ...
    Args:
        ...
        task_type (:obj:`str`, `optional`, defaults to :obj:`"classification"`):
            ...
            Currently, :class:`~text.Trainer` supports two tasks: (1) :obj:`"classification"`, (2) :obj:`"regression"`.
        attack (:class:`~text.Attack`):
            :class:`~text.Attack` used to generate adversarial examples for training.
        train_dataset (:class:`~text.datasets.Dataset`):
            Dataset for training.
        eval_dataset (:class:`~text.datasets.Dataset`):
            Dataset for evaluation
        training_args (:class:`~text.TrainingArgs`):
    ...
    Example::
        ...
        >>> attack = text.attack_recipes.DeepWordBugGao2018.build(model_wrapper)
        >>> train_dataset = text.datasets.HuggingFaceDataset("imdb", split="train")
        >>> eval_dataset = text.datasets.HuggingFaceDataset("imdb", split="test")
        ...
        >>> training_args = text.TrainingArgs(
        ...     num_epochs=3,
        ...     num_clean_epochs=1,
    ...
    .. note::
        When using :class:`~text.Trainer` with `parallel=True` in :class:`~text.TrainingArgs`,
    ...
    def __init__(
        ...
        ), f"`model_wrapper` must be of type `text.models.wrappers.ModelWrapper`, but got type `{type(model_wrapper)}`."
        ...
        if attack:
            assert isinstance(
                attack, Attack
            ), f"`attack` argument must be of type `text.Attack`, but got type of `{type(attack)}`."
        ...
        if train_dataset:
            ...
            ), f"`train_dataset` must be of type `text.datasets.Dataset`, but got type `{type(train_dataset)}`."

        if eval_dataset:
            assert isinstance(
                eval_dataset, textattack.datasets.Dataset
            ), f"`eval_dataset` must be of type `text.datasets.Dataset`, but got type `{type(eval_dataset)}`."

        if training_args:
            assert isinstance(
                training_args, TrainingArgs
            ), f"`training_args` must be of type `text.TrainingArgs`, but got type `{type(training_args)}`."
        else:
            training_args = TrainingArgs()
    ...
    def get_train_dataloader(self, dataset, adv_dataset, batch_size):
        ...
        Args:
            dataset (:class:`~text.datasets.Dataset`):
                Original training dataset.
            adv_dataset (:class:`~text.datasets.Dataset`):
        ...
        for epoch in range(1, self.training_args.num_epochs + 1):
            ...
            if self.attack and epoch > num_clean_epochs:
                if (
                    ...
                    # adv_dataset is instance of `text.datasets.Dataset`
                    model.eval()
