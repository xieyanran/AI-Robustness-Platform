...
class Attack:
    ...
    Args:
        goal_function (:class:`~text.goal_functions.GoalFunction`):
            A function for determining how well a perturbation is doing at achieving the attack's goal.
        constraints (list of :class:`~text.constraints.Constraint` or :class:`~text.constraints.PreTransformationConstraint`):
            A list of constraints to add to the attack, defining which perturbations are valid.
        transformation (:class:`~text.transformations.Transformation`):
            The transformation applied at each step of the attack.
        search_method (:class:`~text.search_methods.SearchMethod`):
    ...
    Example::
        ...
        >>> model = transformers.AutoModelForSequenceClassification.from_pretrained("text/bert-base-uncased-imdb")
        >>> tokenizer = transformers.AutoTokenizer.from_pretrained("text/bert-base-uncased-imdb")
        >>> model_wrapper = text.models.wrappers.HuggingFaceModelWrapper(model, tokenizer)

        >>> # Construct our four components for `Attack`
        >>> from text.constraints.pre_transformation import RepeatModification, StopwordModification
        >>> from text.constraints.semantics import WordEmbeddingDistance
        >>> from text.transformations import WordSwapEmbedding
        >>> from text.search_methods import GreedyWordSwapWIR

        >>> goal_function = text.goal_functions.UntargetedClassification(model_wrapper)
        >>> constraints = [
        ...     RepeatModification(),
        ...
        >>> attack = text.Attack(goal_function, constraints, transformation, search_method)
    ...
    def __init__(
        ...
        assert isinstance(
            goal_function, GoalFunction
        ), f"`goal_function` must be of type `text.goal_functions.GoalFunction`, but got type `{type(goal_function)}`."
        assert isinstance(
            constraints, list
        ), "`constraints` must be a list of `text.constraints.Constraint` or `text.constraints.PreTransformationConstraint`."
        ...
        assert isinstance(
            transformation, Transformation
        ), f"`transformation` must be of type `text.transformations.Transformation`, but got type `{type(transformation)}`."
        assert isinstance(
            search_method, SearchMethod
        ), f"`search_method` must be of type `text.search_methods.SearchMethod`, but got type `{type(search_method)}`."
    ...
    def attack(self, example, ground_truth_output):
        ...
        Args:
            example (:obj:`str`, :obj:`OrderedDict[str, str]` or :class:`~text.shared.AttackedText`):
                ...
                Also accepts :class:`~text.shared.AttackedText` that wraps around the input.
        ...
        Returns:
            :class:`~text.attack_results.AttackResult` that represents the result of the attack.
        """
        assert isinstance(
            example, (str, OrderedDict, AttackedText)
        ), "`example` must either be `str`, `collections.OrderedDict`, `text.shared.AttackedText`."
