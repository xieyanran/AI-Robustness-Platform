...
class Attacker:
    ...
    class uses the :class:`~text.Attack` to actually run the attacks,
    ...
    Args:
        attack (:class:`~text.Attack`):
            :class:`~text.Attack` used to actually carry out the attack.
        dataset (:class:`~text.datasets.Dataset`):
            Dataset to attack.
        attack_args (:class:`~text.AttackArgs`):
    ...
    Example::
        ...
        >>> attack = text.attack_recipes.TextFoolerJin2019.build(model_wrapper)
        >>> dataset = text.datasets.HuggingFaceDataset("imdb", split="test")

        >>> # Attack 20 samples with CSV logging and checkpoint saved every 5 interval
        >>> attack_args = text.AttackArgs(
        ...     num_examples=20,
        ...     log_to_csv="log.csv",
        ...
        >>> attacker = text.Attacker(attack, dataset, attack_args)
    ...
    def __init__(self, attack, dataset, attack_args=None):
        ...
        ), f"`dataset` must be of type `text.datasets.Dataset`, but got type `{type(dataset)}`."

        if attack_args:
            assert isinstance(
                attack_args, AttackArgs
            ), f"`attack_args` must be of type `text.AttackArgs`, but got type `{type(attack_args)}`."
        else:
            attack_args = AttackArgs()
    ...
    def attack_dataset(self):
        ...
        Returns:
            :obj:`list[AttackResult]` - List of :class:`~text.attack_results.AttackResult` obtained after attacking the given dataset..
        """
    ...
    def update_attack_args(self, **kwargs):
        ...
        for k in kwargs:
            if hasattr(self.attack_args, k):
                ...
                raise ValueError(f"`text.AttackArgs` does not have field {k}.")
    ...
    def from_checkpoint(cls, attack, dataset, checkpoint):
        ...
        Args:
            ...
            dataset (:class:`~text.datasets.Dataset`):
                Dataset to attack.
            checkpoint (:obj:`Union[str, :class:`~text.shared.AttackChecpoint`]`):
                Path of saved checkpoint or the actual saved checkpoint.
        """
        assert isinstance(
            checkpoint, (str, textattack.shared.AttackCheckpoint)
        ), f"`checkpoint` must be of type `str` or `text.shared.AttackCheckpoint`, but got type `{type(checkpoint)}`."
