...
class AttackResumeCommand(TextAttackCommand):
    ...
    def run(self, args):
        ...
        assert isinstance(checkpoint.attack_args, CommandLineAttackArgs), (
            f"Expect `attack_args` to be of type `text.args.CommandLineAttackArgs`, but got type `{type(checkpoint.attack_args)}`. "
            f"If saved `attack_args` is not of type `text.args.CommandLineAttackArgs`, cannot resume attack from command line."
        )
