...
TEXTATTACK_DATASET_BY_MODEL = {
    ...
    #
    "t5-en-de": (
        "text.datasets.helpers.TedMultiTranslationDataset",
        "en",
        "de",
    ),
    "t5-en-fr": (
...
class DatasetArgs:
    ...
    def _create_dataset_from_args(cls, args):
        ...
        ``text.dataset.Dataset`` object."""

        assert isinstance(
            args, cls
        ), f"Expect args to be of type `{type(cls)}`, but got type `{type(args)}`."

        # Automatically detect dataset for huggingface & text models.
        ...
        if args.dataset_by_model in HUGGINGFACE_DATASET_BY_MODEL:
            ...
            if dataset[0].startswith("text"):
                # unsavory way to pass custom dataset classes
                # ex: dataset = ('text.datasets.helpers.TedMultiTranslationDataset', 'en', 'de')
        ...
        ), "Loaded `dataset` must be of type `text.datasets.Dataset`."
